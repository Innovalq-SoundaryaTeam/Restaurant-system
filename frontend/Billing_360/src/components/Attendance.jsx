...
const Attendance = () => {
  ...
  try {
    const res = await axios.get("http://localhost:8000/staff/get-staff");
...
useEffect(() => {
        ...
        <Container fluid className="py-3">
          ...
          <Card className="att-dark-card border-0 p-4 mb-5">
            ...
            <Row className="g-3 align-items-end">
              <Col md={3}><><Form.Control list="staff-options" className="att-input-white" placeholder="Name..." value={adminEntry.name} onChange={(e) => setAdminEntry({ ...adminEntry, name: e.target.value })} /><datalist id="staff-options"> {staffList.map((staff) => (<option key={staff.id} value={staff.name} />))} </datalist> </></Col>
  ...
  onClick={() => { setCurrentEditRecord(staff); setShowEditModal(true); }}
>
  <FaPencilAlt />
</Button>
