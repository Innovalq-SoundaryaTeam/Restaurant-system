...
@router.post("/billing/generate-session-bill/{session_id}")
def generate_session_bill(session_id: str, db: Session = Depends(get_db)):
    ...
    if not session:
        raise HTTPException(status_code=400, detail="Session already closed or not found")
